...
/* Vector SEXP: memory is laid out as [SEXPREC_HEADER | vecsxp | DATA...]
   Vector data immediately follows the vecsxp struct and is accessed via
   STDVEC_DATAPTR macro. For R_xlen_t values: length is current size,
   truelength is allocated capacity (for growth without reallocation). */
typedef struct VECTOR_SEXPREC {
    SEXPREC_HEADER;              /* type, attributes, GC info */
    struct vecsxp_struct vecsxp; /* length and truelength metadata */
