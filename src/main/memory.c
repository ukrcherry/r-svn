...
/* Helper macro to get element size for vector type */
#define VECTOR_TYPE_ELEMENT_SIZE(type) \
    ((type) == RAWSXP ? sizeof(Rbyte) : \
     (type) == LGLSXP || (type) == INTSXP ? sizeof(int) : \
     (type) == REALSXP ? sizeof(double) : \
     (type) == CPLXSXP ? sizeof(Rcomplex) : \
     (type) == STRSXP || (type) == EXPRSXP || (type) == VECSXP ? sizeof(SEXP) : \
     0)

/* Valgrind instrumentation helper for vector data initialization */
#if VALGRIND_LEVEL > 0
static inline void valgrind_mark_vector_undefined(SEXP s, R_size_t actual_size) {
    if (actual_size > 0)
        VALGRIND_MAKE_MEM_UNDEFINED(STDVEC_DATAPTR(s), actual_size);
}
#define MARK_VECTOR_UNDEFINED(s, sz) valgrind_mark_vector_undefined(s, sz)
#else
#define MARK_VECTOR_UNDEFINED(s, sz) ((void)0)
#endif

/* Helper function to ensure sufficient heap space for allocation */
static void ensure_alloc_heap_space(R_size_t needed_size) {
    if (FORCE_GC || NO_FREE_NODES() || VHEAP_FREE() < needed_size) {
        R_gc_internal(needed_size);
        if (NO_FREE_NODES())
            mem_err_cons();
        if (VHEAP_FREE() < needed_size)
            mem_err_heap(needed_size);
    }
}
...
SEXP allocVector3(SEXPTYPE type, R_xlen_t length, R_allocator_t *allocator)
{
	...
	case LGLSXP:
	    ...
	    ensure_alloc_heap_space(alloc_size);

	    CLASS_GET_FREE_NODE(node_class, s);
	    {
		R_size_t elem_size = VECTOR_TYPE_ELEMENT_SIZE(type);
		MARK_VECTOR_UNDEFINED(s, elem_size);
	    }
	    s->sxpinfo = UnmarkedNodeTemplate.sxpinfo;
	    SETSCALAR(s, 1);
	    SET_NODE_CLASS(s, node_class);
	...
	if (node_class < NUM_SMALL_NODE_CLASSES) {
	    CLASS_GET_FREE_NODE(node_class, s);
	    MARK_VECTOR_UNDEFINED(s, actual_size);
	    s->sxpinfo = UnmarkedNodeTemplate.sxpinfo;
	    INIT_REFCNT(s);
	    SET_NODE_CLASS(s, node_class);
	    R_SmallVallocSize += alloc_size;
	    ...
	    /* Validate that total allocation size won't overflow size_t.
	       Since size is already validated above, check if adding header
	       and converting to bytes would overflow. */
